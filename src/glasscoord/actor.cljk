(ns glasscoord.actor
  "GlassCoordActor — the ISCO-08 7315 glass makers, cutters, grinders
  and finishers glass-workshop scheduling/logistics coordination
  actor as a `langgraph.graph/state-graph` (ADR-2607121000 /
  CLAUDE.md Actors section). One graph run = one glass-workshop
  coordination operation request (intake -> advise -> govern ->
  ...
  Modeled closely on cloud-itonami-isco-7211's foundrycoord.actor,
  the closest analogous hot-process/heat-exposure workshop safety
  pattern.
  ...
  The unconditional invariant: the Glass Workshop Coordination
  Advisor can never directly commit an operating record the
  GlassCoordGovernor refuses, and can never make this actor finalize
  a glass-forming/cutting-execution decision (e.g. deciding to
  proceed with a specific glass-forming or glass-cutting operation)
  or override a workshop safety officer's judgment — every
  ...
  This actor coordinates GLASS-WORKSHOP SCHEDULING/LOGISTICS ONLY —
  it never performs glass-forming, -cutting, -grinding or -finishing
  work or authorizes glass-forming/cutting operations itself."
...
(defn build-graph
  "Build a compiled GlassCoordActor graph. `store` implements
  `glasscoord.store/Store`. `advisor` implements
  `glasscoord.advisor/Advisor` (defaults to `mock-advisor`).
  ...
  (-> (g/state-graph
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:glassworker-id (:glassworker-id request)
                                    :op (:op proposal)
